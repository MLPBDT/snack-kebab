    ...
    <div className="ks-root" style={{ minHeight: '100%', position: 'relative' }}>
      ...
      <section style={{ padding: `${isMobile ? 56 : 96}px ${PX}px`, background: palette.bg }}>
        <div style={{ maxWidth: 1280, margin: '0 auto' }}>
          ...
          <h2 className="ks-display" style={{ fontSize: isMobile ? 40 : 64, margin: '0 0 32px', color: palette.text }}>4.8 sur 320 avis.</h2>
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3, 1fr)', gap: 16 }}>
            {[
              ...
              <div key={i} className="ks-card" style={{ padding: 22 }}>
                <div style={{ display: 'flex', gap: 1, color: palette.accent, marginBottom: 12 }}>
                  {Array(r.s).fill(0).map((_, j) => <span key={j}>{I.star}</span>)}
                </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, paddingTop: 14, borderTop: `1px solid ${palette.text}10` }}>
                  <div style={{ width: 36, height: 36, borderRadius: 999, background: palette.primary, color: palette.onPrimary, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 700 }}>{r.n[0]}</div>
                  <div>
